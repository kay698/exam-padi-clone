...
const QandA = ({item}) => {
  const [WaecJambContent,setWaecJambContent] = useContext(CbtWJContext)
    ...
    return ( 
        ...
        <FlexibleDiv justifyContent="flex-end">
           {WaecJambContent === 'practice' ? <Button onClick={() => handleViewAnswer()} style={{color: "#5B64B1", marginTop:"none"}}>View answer</Button> : <div />}
            </FlexibleDiv>
    ...
    <FlexibleDiv justifyContent="flex-start" style={{padding: "40px 20px 0 20px", color: "#27AE60"}}>
       {WaecJambContent === "practice" &&( viewAnswer ? <><p>Correct answers</p> <p style={{marginLeft: "20px"}}>{viewAnswer}</p></>
       : null )}
    </FlexibleDiv>
     </FlexibleDiv>
    )
